...
public class ERC721TransactionsClient extends EtherscanClient<EthTokenTransaction> {

    private static final String ACTION = "tokennfttx";

    public ERC721TransactionsClient(String apiKey) {
        super(apiKey);
    }
    ...
    public List<EthTokenTransaction> getTransactions(String walletAddress, TransactionsRequest request) {
        try {
            ...
            if (response != null) {
                return response
                        .stream()
                        .map(Converter::getEthERC721Transaction)
                        .collect(Collectors.toList());
            }
        } catch(Exception e) {
            String msg = "Error getting data from getERC721TxnUrl API";
